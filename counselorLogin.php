...
if ($isLoggedIn) {
    $util->redirect("counselorPage.php");
}
...
if (! empty($_POST["login"])) {
    ...
    $staff = $auth->getStaffByID($staffid);

    if ( $staff == null){
      $message = "Invalid Login";
    }

    else if (password_verify($password, $staff[0]["password"])) {
    ...
    if ($isAuthenticated) {
        ...
        if (! empty($_POST["remember"])) {
            setcookie("login", $staffid, $cookie_expiration_time);
            ...
            $userToken = $auth->getTokenByUserID($staffid, 0);
            if (! empty($userToken[0]["id"])) {
                $auth->markAsExpired($userToken[0]["id"]);
            }
            // Insert new token
            $auth->insertToken($staffid, $random_password_hash, $random_selector_hash, $expiry_date);
        } else {
...
<form action="" method="post" id="frmLogin">
    ...
    <div class="field-group">
        ...
        <div>
            <input name="staffid" type="text" placeholder="Staff ID"
